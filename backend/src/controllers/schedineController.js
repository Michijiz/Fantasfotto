...
// consegnato e con che quota, ma non i pronostici: lo scontrino resta segreto
...
const perGiornata = async (req, res) => {
  ...
  if (!chiusa(grezza)) {
    const partecipanti = schedine.map((s) => ({ utente: s.utente, squadra: s.squadra, quotaTotale: s.quotaTotale }));
