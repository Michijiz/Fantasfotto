...
Non c'è più un seed automatico delle squadre: si creano una alla volta dall'app (pagina
Squadre, solo admin) o via `POST /api/squadre` con `{ "nome": "..." }`. Al primo avvio,
prima che esista qualsiasi utente/admin, serve un piccolo bootstrap manuale (una volta
sola):

1. Su Atlas → Browse Collections, crea a mano un documento nella collection `squadras`
   con solo `{ "nome": "La tua prima squadra" }` (basta uno, giusto per poterti
   registrare).
2. Iscriviti dall'app scegliendo quella squadra.
3. Torna su Atlas, apri il tuo utente in `users` e cambia `ruolo` da `giocatore` ad
   `admin`.
4. Da qui in poi puoi creare tutte le altre squadre direttamente dall'app.
